...
public class GameModel implements Cloneable{
   ...
   /**
    ...
    /**
     * The clone of the gameModel.
     */
    private GameModel gameModelClone;
    ...
    /**
     * Cloning method used to clone the gamemodel and specific reference variables.
     */
   @Override
   protected Object clone() throws CloneNotSupportedException{
	   
	   gameModelClone = (GameModel)super.clone();
	   gameModelClone.numberOfSteps = getNumberOfSteps();
   	   gameModelClone.currentDot = (Point)currentDot.clone();
   	   int[][] tempmodel = new int[getSize()][getSize()];
   	   
   	   for(int i = 0; i < getSize(); i++){
   		   for(int j = 0; j < getSize(); j++){
   			   tempmodel[i][j] = getCurrentStatus(i,j);
   		   }
   	   }
   	   
   	   gameModelClone.model = tempmodel;
   	   gameModelClone.setCurrentDot(currentDot.getX(), currentDot.getY());
   	  
	   return gameModelClone; 
   }
